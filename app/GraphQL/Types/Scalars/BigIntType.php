...
class BigIntType extends ScalarType implements GraphQlType, TypeConvertible
{
    ...
    {
        parent::__construct(['description' => __('type.big_int.description')]);
    }
    ...
    public function parseValue($value)
    {
        if (! $this->isValid($value)) {
            throw new Error(__('error.cannot_represent_uint256', ['value' => Utils::printSafeJson($value)]));
        }
    ...
    public function parseLiteral($valueNode, ?array $variables = null)
    {
        if (! in_array($valueNode->kind, ['IntValue', 'StringValue']) || ! $this->isValid($valueNode->value)) {
            throw new Error(__('error.not_valid_uint256'), [$valueNode]);
        }
